...
// regular death
	...
	if (game_flag == 1) {
		...
		if (zombie_kills > high_score) {
			FILE *fp;
			fp = fopen("highscore.txt", "w+");

			high_score = zombie_kills;
...
void spawn_wave(int wave){
	...
	for (int i = 0; i < wave; i++) {
		...
		if (i % 2 == 0) {
			edict_t *loc = G_Spawn();
			loc->s.origin[0] = 100 + (i * 40);
			loc->s.origin[1] = -100 + (i * 20);
			loc->s.origin[2] = 25;

			loc->s.angles[1] = 270;

			SP_monster_soldier(loc);
		}
		else {
			edict_t *loc = G_Spawn();
			loc->s.origin[0] = 50 + (i * 10);
			loc->s.origin[1] = -200 - (i * 20);
			loc->s.origin[2] = 25;
